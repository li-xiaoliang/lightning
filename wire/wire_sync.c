...
{
	be16 be_len = cpu_to_le16(tal_count(msg));

	assert(le16_to_cpu(be_len) == tal_count(msg));
...
u8 *wire_sync_read(const tal_t *ctx, int fd)
{
	...
	if (!read_all(fd, &be_len, sizeof(be_len)))
		return NULL;
	msg = tal_arr(ctx, u8, le16_to_cpu(be_len));
	if (!read_all(fd, msg, le16_to_cpu(be_len)))
		return tal_free(msg);
	return msg;
}
