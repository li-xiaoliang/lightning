...
#include <errno.h>
#include <lightningd_handshake/gen_handshake_control_wire.h>
#include <lightningd_handshake/gen_handshake_status_wire.h>
			  ...
			  const void *in2, size_t in2_size)
{
	/* BOLT #8:
	 *
	 *   * `HKDF`: a function is defined in [5](#reference-5), evaluated
	 *      with a zero-length `info` field.
 ...
 *        variant) with the passed arguments, with nonce `n` encoded as a
 *        big-endian 96-bit value.
 */
...
static void act_two_initiator(struct handshake *h, int fd, struct pubkey *re)
{
	...
	if (!read_all(fd, &act2, ACT_TWO_SIZE))
		status_failed(WIRE_INITR_ACT2_READ_FAILED,
			      "%s", strerror(errno));
	status_trace("act2=%s", tal_hexstr(trc, &act2, ACT_TWO_SIZE));
...
static struct crypto_complete *initiator(const tal_t *ctx,
					 int fd, const struct pubkey *my_id,
					 const struct pubkey *their_id)
{
	struct crypto_complete *cc = tal(ctx, struct crypto_complete);
	struct handshake *h = new_handshake(cc, their_id);
	struct pubkey re;
	 ...
	 *        session.
	 */
	hkdf_two_keys(&cc->sk, &cc->rk, &cc->ck, NULL, 0);
	status_trace("output: sk,rk=0x%s,0x%s",
		     tal_hexstr(trc, &cc->sk, sizeof(cc->sk)),
		     tal_hexstr(trc, &cc->rk, sizeof(cc->rk)));
	return cc;
}

static struct crypto_complete *responder(const tal_t *ctx,
					 int fd,
					 const struct pubkey *my_id,
					 struct pubkey *their_id)
{
	struct crypto_complete *cc = tal(ctx, struct crypto_complete);
	struct handshake *h = new_handshake(cc, my_id);
	struct pubkey re;
	 ...
	 *        session.
	 */
	hkdf_two_keys(&cc->rk, &cc->sk, &cc->ck, NULL, 0);
...
int main(int argc, char *argv[])
{
	...
	if (fromwire_handshake_responder_req(msg, NULL, &my_id)) {
		cc = responder(msg, CLIENT_FD, &my_id, &their_id);
		wire_sync_write(REQ_FD,
				towire_handshake_responder_resp(msg,
						    ...
						    &their_id)) {
		cc = initiator(msg, CLIENT_FD, &my_id, &their_id);
		wire_sync_write(REQ_FD,
				towire_handshake_initiator_resp(msg,
								&cc->ck.s,
								&cc->rk.s,
								&cc->sk.s));
	} else
