...
#include <fcntl.h>
#include <inttypes.h>
#include <lightningd_hsm/client.h>
#include <lightningd_hsm/gen_hsm_client_wire.h>
#include <lightningd_hsm/gen_hsm_control_wire.h>
#include <lightningd_hsm/gen_hsm_status_wire.h>
					   ...
					   struct conn_info *ci)
{
	...
	status_trace("Client %"PRIu64": type %i len %zu",
		     c->id, fromwire_peektype(ci->in), tal_count(ci->in));
	...
	return io_write(conn, ci->out, tal_count(ci->out), sent_resp, ci);
}
...
{
	int fd = open("hsm_secret", O_CREAT|O_EXCL|O_WRONLY, 0600);
	if (fd < 0)
		status_failed(WIRE_HSMSTATUS_INIT_FAILED,
			      "creating: %s", strerror(errno));

	randombytes_buf(&hsm_secret, sizeof(hsm_secret));
	if (write(fd, &hsm_secret, sizeof(hsm_secret)) != sizeof(hsm_secret)) {
...
{
	int fd = open("hsm_secret", O_RDONLY, 0600);
	if (fd < 0)
		status_failed(WIRE_HSMSTATUS_INIT_FAILED,
...
static struct io_plan *received_reqlen(struct io_conn *conn,
				       struct conn_info *ci)
{
	ci->in = tal_arr(ci, u8, le16_to_cpu(ci->inlen));
	return io_read(conn, ci->in, tal_count(ci->in), ci->received_req, ci);
}

static struct io_plan *recv_req(struct io_conn *conn, struct conn_info *ci)
{
	return io_read(conn, &ci->inlen, sizeof(ci->inlen), received_reqlen, ci);
}
			   ...
			   const tal_t *data)
{
	...
	/* We don't have an async way to send fds, so we do it sync. */
	if (!fdpass_send(io_conn_fd(conn), fds[1]))
		status_failed(WIRE_HSMSTATUS_FD_FAILED,
			      "passing fd: %s", strerror(errno));
	close(fds[1]);
...
static struct io_plan *control_received_req(struct io_conn *conn,
					    struct conn_info *control)
{
	enum control_wire_type t = fromwire_peektype(control->in);

	status_trace("Control: type %i len %zu", t, tal_count(control->in));
...
send_out:
	/* If we have any response, send it, otherwise just read next. */
	if (control->out)
		return io_write(conn, control->out, tal_count(control->out),
				sent_resp, control);
	else
